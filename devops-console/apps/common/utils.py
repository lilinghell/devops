...
def git(scm_url, auth_token):
    url_res = urlparse(scm_url)
    url = url_res.scheme + "://" + url_res.netloc
    gl = gitlab.Gitlab(url, auth_token)
    path = url_res.path.replace(".git", "").replace("/", "", 1)
    return gl, path, url
...
def upload_file_path(instance, filename):
    path = settings.UPLOAD_FILE_DIR
    today = datetime.datetime.today().strftime('%Y%m%d')
    format = uuid.uuid4().hex + '.' + filename.split('.')[-1]
    return os.path.join(path, today, format)
...
def get_logger(name=None):
    return logging.getLogger('devops.%s' % name)
